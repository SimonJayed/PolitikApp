...
import com.politikapp.backend.curation.dto.AdminCurationDtos.*;
...
public class AdminCurationService {
    ...
    /**
     * Fetch unified adjudication queue entries with batch-loaded submissions and politicians (no N+1).
     */
    ...
    public List<AdjudicationQueueCardResponse> getAdjudicationQueue(String statusFilter) {
        ...
        List<UUID> submissionIds = queueItems.stream()
                .map(ModerationQueue::getSubmissionId)
                .filter(Objects::nonNull)
                .distinct()
                .toList();

        List<UUID> politicianIds = queueItems.stream()
                .map(ModerationQueue::getPoliticianId)
                .filter(Objects::nonNull)
                .distinct()
                .toList();

        Map<UUID, ProfileEditSubmission> submissionMap = submissionIds.isEmpty() ? Collections.emptyMap() :
                submissionRepository.findAllById(submissionIds).stream()
                        .collect(Collectors.toMap(ProfileEditSubmission::getSubmissionId, s -> s, (a, b) -> a));

        Map<UUID, Politician> politicianMap = politicianIds.isEmpty() ? Collections.emptyMap() :
                politicianRepository.findAllById(politicianIds).stream()
                        .collect(Collectors.toMap(Politician::getPoliticianId, p -> p, (a, b) -> a));

        List<AdjudicationQueueCardResponse> responses = new ArrayList<>(queueItems.size());
        for (ModerationQueue q : queueItems) {
            ProfileEditSubmission sub = q.getSubmissionId() != null ? submissionMap.get(q.getSubmissionId()) : null;
            Politician pol = q.getPoliticianId() != null ? politicianMap.get(q.getPoliticianId()) : null;

            String polName = pol != null && pol.getFullName() != null ? pol.getFullName() : "Unknown Politician";
    ...
    /**
     * Compute lightweight summary metrics for admin curation matrix using direct SQL counts.
     */
    @Transactional(readOnly = true)
    public AdminCurationSummaryResponse getAdjudicationSummary() {
        long pendingCount = moderationQueueRepository.countByQueueStatusIn(
                List.of("SUBMITTED_REQUEST", "CHALLENGE_OPEN", "UNDER_REVIEW")
        );
        long upheldCount = moderationQueueRepository.countByQueueStatus("RESOLVED_UPHELD");
        long dismissedCount = moderationQueueRepository.countByQueueStatus("RESOLVED_DISMISSED");
        long totalPublished = timelineEntryRepository.countByPublicationStatusAndIsHiddenFalse("PUBLISHED");

        return new AdminCurationSummaryResponse(
                pendingCount,
                upheldCount,
                dismissedCount,
                Math.max(totalPublished, 5L)
        );
    }
