...
alter table public.politicians add column if not exists wikidata_id varchar(50);
alter table public.politicians add column if not exists source_url text;
alter table public.politicians add column if not exists last_synced_at timestamp;
alter table public.politicians alter column term_start drop not null;
alter table public.politicians alter column term_end drop not null;
...
create unique index if not exists idx_politicians_wikidata_id_unique
  on public.politicians(wikidata_id)
